...
data Error = InvalidTopic | InvalidCommentText | InvalidRequest
...
mkTopic
  :: Text
  -> Either Error Topic
mkTopic "" = Left InvalidTopic
mkTopic topic = if Text.length topic < 5 then Left InvalidTopic else Right $ Topic topic
...
mkCommentText "" = Left InvalidCommentText
mkCommentText commentText = Right $ CommentText commentText

getCommentText
  :: CommentText
  -> Text
getCommentText (CommentText commentText) = commentText
